    ...
    do {
        ...
        scanf(" %d", &n);
            ...
            /* Her allokeres space til mit scoreboard array, som skal være 16 pladser stort.
               ...
               kunne gemme værdien for 15 runders score + en samlet sum på plads 0.         */
...
void Play_Yatzy(int n, int dice_rolls[], int score_board[])
{
    ...
    for (round_number = 1; round_number <= 15; ++round_number)
    {
        switch (round_number)
        {
            ...
            case One_Pair:
                Print_New_Line_in_Table();
                Check_for_Bonus(score_board);   /* Her tjekker jeg for bonus, før jeg spiller runde 7. */
                Print_New_Line_in_Table();
                printf("|| Round number %4d ", round_number);
...
void Find_Numbers_One_to_Six(int n, int dice_rolls[], int round_number, int score_board[])
{
    ...
    gemmes tallet ikke. Jeg sørger endvidere for, at jeg ikke kan få flere
    end 5 af samme terning gemt i score vha. min variabel, som hedder
    selected_amount_of_dice.                                              */
    for (i = 0; i < n; ++i)
    {
        if (dice_rolls[i] == round_number)
        {
            ...
            {
                i = n; /* Breaker ud af mit loop ved 5 fundne terninger */
            }
        }
    }  
    ...
    for (i = (n - 1); i > 0; --i)
        {
            if(dice_rolls[i] == dice_rolls[i - 1] && (pairs < 2) && (dice_rolls[i] != block_same_pair))
                {
                block_same_pair = dice_rolls[i]; /* Min block_same_pair får værdien af det første par fundet, så næste par ikka kan være det samme. */
